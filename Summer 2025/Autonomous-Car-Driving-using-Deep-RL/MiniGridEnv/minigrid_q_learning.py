...
def q_learning_train(env, Q, episodes, alpha, gamma, n_actions):
    rewards = []
    steps = []
    epsilon = 1.0
    epsilon_min = 0.001
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            td_target = reward + gamma * np.max(Q[next_state]) - Q[state][action]
...
Q, rewards, steps = q_learning_train(env,Q,episodes=1000,alpha=0.1,gamma=0.95,n_actions=n_actions)
